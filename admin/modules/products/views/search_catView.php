...
// $list_product_cat = data_tree($data_cat, 0);

// 
$total_all = db_num_rows("SELECT * FROM `tbl_product_cat`");
$total_approved = db_num_rows("SELECT * FROM `tbl_product_cat` WHERE `cat_status` = 'approved'");
$total_pending = db_num_rows("SELECT * FROM `tbl_product_cat` WHERE `cat_status` = 'pending'");
        ...
        <div id="content" class="fl-right">
            <div class="section" id="title-page">
                <div class="clearfix">
                    ...
                    <a href="?mod=products&controller=category&action=add_cat" title="" id="add-new" class="fl-left">Thêm
                        mới</a>
                </div>
            </div>
                ...
                <div class="section-detail">
                    <div class="filter-wp clearfix">
                        <ul class="post-status fl-left p-0">
                            <li class="all"><a href="?mod=products&controller=category&action=list_cat">Tất cả <span
                    ...
                    <form method="POST" action="?mod=products&controller=category&action=apply_status"
                        class="form-actions">
                        <div class="actions">
                            <select name="actions">
                        ...
                        <div class="table-responsive">
                            <table class="table list-table-wp">
                                ...
                                <?php if (!empty($list_search_cat)) {
                                    $t = 0; ?>
                                    <tbody>
                                        <?php foreach ($list_search_cat as $product_cat) {
                                            $t++; ?>
                                            <tr>
                                                <td><input type="checkbox" name="checkItem[]" class="checkItem"
                                                        value="<?php echo $product_cat['cat_id']; ?>"></td>
                                                <td><span class="tbody-text"><?php echo $t; ?></h3></span>
                                                <td class="clearfix">
                                                    <div class="tb-title fl-left">
                                                        <a href="?mod=products&controller=category&action=update_cat&cat_id=<?php echo $product_cat['cat_id']; ?>"
                                                            title=""><?php echo $product_cat['cat_title'] ?></a>
                                                    </div>
                                                    <ul class="list-operation fl-right">
                                                        <li><a href="?mod=products&controller=category&action=update_cat&cat_id=<?php echo $product_cat['cat_id']; ?>"
                                                                title="Sửa" class="edit"><i class="fa fa-pencil"
                                                                    aria-hidden="true"></i></a></li>
                                                        <li><a href="?mod=products&controller=category&action=delete_cat&cat_id=<?php echo $product_cat['cat_id']; ?>"
                                                                title="Xóa" onclick="return confirm('Bạn có chắc thực hiện thao tác này?')" class="delete"><i class="fa fa-trash"
                                                                    aria-hidden="true"></i></a></li>
                                                    </ul>
                                                </td>
                                                <td><span class="tbody-text"><?php echo $product_cat['num_order']; ?></span></td>
                                                <td><span
                                                        class="tbody-text <?php echo status_color($product_cat['cat_status']); ?>"><?php echo $product_cat['cat_status']; ?></span>
                                                </td>
                                                <td><span class="tbody-text"><?php echo $product_cat['creator']; ?></span></td>
                                                <td><span class="tbody-text"><?php echo $product_cat['created_date']; ?></span>
                                                </td>
                                    ...
                                    $error['product_cat'] = "Không có bản ghi nào!";
                                    ?>
